...
export const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-zinc-900/60 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm transition-colors">
        
        {/* Month Navigation */}
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-emerald-500/10 text-emerald-500 font-bold flex items-center justify-center">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          ...
          {/* Total Monthly Hours */}
          <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 flex flex-col justify-center">
            <div className="flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-400 font-medium">
              <Clock className="w-3.5 h-3.5 text-cyan-500" />
              Hours Spent
            </div>
            <p className="text-sm sm:text-base font-black text-zinc-900 dark:text-white mt-1">
          ...
          <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 flex flex-col justify-center">
            ...
            <p className="text-sm sm:text-base font-black text-zinc-900 dark:text-white mt-1">
              {stats.totalGymDays} <span className="text-xs font-normal text-zinc-400">/ {stats.daysInMonth}</span>
            </p>
          </div>
          ...
          <div className="bg-zinc-50 dark:bg-zinc-950 p-3 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 flex flex-col justify-center">
            ...
            <p className="text-sm sm:text-base font-black text-zinc-900 dark:text-white mt-1">
              {stats.totalRestDays} <span className="text-xs font-normal text-zinc-400">days</span>
            </p>
          </div>
      ...
      {/* Calendar Grid */}
      <div className="bg-white dark:bg-zinc-900/60 p-4 sm:p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
        
        {/* Days of Week Header */}
        <div className="grid grid-cols-7 text-center mb-3 text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">
          <div>Sun</div>
          <div>Mon</div>
          <div>Tue</div>
          <div>Wed</div>
        ...
        <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
          ...
          {Array.from({ length: firstDayOfMonth }).map((_, idx) => (
            <div key={`blank-${idx}`} className="h-20 sm:h-24 rounded-xl bg-zinc-50/40 dark:bg-zinc-950/20 opacity-30" />
          ))}
          ...
          {Array.from({ length: daysInMonth }).map((_, idx) => {
            ...
            return (
              <div
                ...
                className={`group relative h-20 sm:h-24 p-2 rounded-xl border transition-all flex flex-col justify-between cursor-pointer ${
                  isToday
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 rounded-md bg-emerald-500 text-white hover:bg-emerald-600 transition-opacity"
                ...
                <div className="space-y-1 overflow-hidden">
                  {hasWorkout ? (
                    dayWorkouts.map((w) => (
                      <div
                        key={w.id}
                        className="truncate text-[10px] sm:text-xs px-1.5 py-0.5 rounded-md bg-gradient-to-r from-emerald-500/15 to-cyan-500/15 text-emerald-600 dark:text-emerald-400 font-medium border border-emerald-500/20 flex items-center gap-1"
                      >
                    ...
                    <div className="text-[10px] text-zinc-400 dark:text-zinc-600 flex items-center gap-1 italic opacity-75">
      ...
      <div className="bg-white dark:bg-zinc-900/60 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-4">
        <h3 className="font-extrabold text-base text-zinc-900 dark:text-white flex items-center justify-between">
